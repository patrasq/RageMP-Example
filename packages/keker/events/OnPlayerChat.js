...
module.exports =
{
	...
	{
		const str = "<font color='red'>" + player.name + "</font>: <b>" + striptags(text, '<b><s><u><strong>') + "</b>";
        mp.players.broadcastInRange(player.position, 25, 'Message from ${player.name}: </b>' + striptags(text, '<b><s><u><strong>'));
        mp.players.broadcastInRange(player.position, 25.0, text);
	}
}